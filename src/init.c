...
//	_sysio_mount_root(ty, name, flags, v[3].ovi_value);
...
//	arg = "{mnt, dev=\"fuse:\", dir=/, fl=2}";
//	err = _sysio_boot("namespace", arg);
	err = _sysio_mount_root(NULL, "fuse", 2, NULL);
	if (err)
		return err;

//	arg = "{mnt, dev=\"fuse:/data/yzhu/bbfs\", dir=/b_fuse/, fl =2}";
//	err = _sysio_boot("namespace", arg);
//	if (err)
//		return err;
